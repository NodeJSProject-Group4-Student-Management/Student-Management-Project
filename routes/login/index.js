...
const login = async (req,res) => {
    ...
    try{
        const user = await pool.query("SELECT * FROM admin WHERE username=$1",[username]) //once kayıtlı username var mı yok mu kontrolü.
        if(user.rowCount==0){
...
const register = async (req,res) => {
    ...
    try{
        const checkUserName = await pool.query("SELECT * FROM admin WHERE username=$1",[username]);
        if(checkUserName.rowCount!=0){
            return res.status(409).json({msg:"Bu kullanici adi daha once alinmis"});
        }
        const hashedPW = await bcrypt.hash(password,10)
        const user = await pool.query("INSERT INTO admin (username,password) values ($1,$2)",[username,hashedPW])

        if(user.rowCount!=0){
            res.status(201).json({msg:"Kayıt başarıyla oluşturuldu."})
        }
